    ...
    public class UserStatusChangeServiceErrorHandler : ErrorResponseHandler, IUserStatusChangeServiceErrorHandler
    {
            ...
            out Exception generatedException)
        {
            ...
            foreach (var errorItem in errors)
            {
                ...
                {
                    generatedException = new ApplyCodeNotFoundException(Covi.Features.ApplyCode.Resources.Localization.ApplyCodeNotFoundException_ErrorText);
                    return true;
                }

                if (ResponseErrorCode.AcceptStatusCodeExpired.IsError(errorItem.ErrorTarget))
                {
                    generatedException = new ApplyCodeExpiredException(Covi.Features.ApplyCode.Resources.Localization.ApplyCodeExpiredException_ErrorText);
                    return true;
                }
            }
